...
bool ExynosCameraActivityControl::autoFocus(int focusMode, int focusType)
{
    ...
    if (m_flashMgr->getNeedCaptureFlash() == true) {
        /* start Pre-flash */
        ret = startPreFlash(this->getAutoFocusMode());
...
int ExynosCameraActivityControl::startMainFlash(void)
{
    ...
    if (m_flashIsCanceled == true) {
        ALOGD("DEBUG(%s): m_flashIsCanceled(%d)", __FUNCTION__, m_flashIsCanceled);
        m_flashIsCanceled = false;
        return -1;
    }
