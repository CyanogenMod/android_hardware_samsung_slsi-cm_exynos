...
status_t ExynosCameraFrameFactoryFront::create(void)
{
    ...
    nodeNums[OUTPUT_NODE] = FRONT_CAMERA_3AA_NUM;
    nodeNums[CAPTURE_NODE] = FRONT_CAMERA_3AA_NUM;
    nodeNums[SUB_NODE] = -1;
    m_pipes[INDEX(PIPE_3AA_FRONT)] = (ExynosCameraPipe*)new ExynosCameraPipe3AA(m_cameraId, m_parameters, false, nodeNums);
    m_pipes[INDEX(PIPE_3AA_FRONT)]->setPipeId(PIPE_3AA_FRONT);
    m_pipes[INDEX(PIPE_3AA_FRONT)]->setPipeName("PIPE_3AA_FRONT");

    nodeNums[OUTPUT_NODE] = FRONT_CAMERA_3AA_NUM + 1;
    nodeNums[CAPTURE_NODE] = FRONT_CAMERA_3AA_NUM + 1;
    nodeNums[SUB_NODE] = -1;
    m_pipes[INDEX(PIPE_3AC_FRONT)] = (ExynosCameraPipe*)new ExynosCameraPipe3AC(m_cameraId, m_parameters, false, nodeNums);
    m_pipes[INDEX(PIPE_3AC_FRONT)]->setPipeId(PIPE_3AC_FRONT);
    m_pipes[INDEX(PIPE_3AC_FRONT)]->setPipeName("PIPE_3AC_FRONT");
    ...
    sensorIds[OUTPUT_NODE] = (0 << REPROCESSING_SHIFT)
                   ...
                   | ((FRONT_CAMERA_3AP_NUM - FIMC_IS_VIDEO_SS0_NUM) << TAX_VINDEX_SHIFT)
                   | (sensorId << 0);
    ...
    /* 3AA pipe initialize */
    sensorIds[OUTPUT_NODE] = sensorId;
    sensorIds[CAPTURE_NODE] = sensorId;
    sensorIds[SUB_NODE] = -1;
    ret = m_pipes[INDEX(PIPE_3AA_FRONT)]->create(sensorIds);
    if (ret < 0) {
        ALOGE("ERR(%s[%d]):3AA create fail, ret(%d)", __FUNCTION__, __LINE__, ret);
        /* TODO: exception handling */
        return INVALID_OPERATION;
    }
    ALOGD("DEBUG(%s):Pipe(%d) created", __FUNCTION__, INDEX(PIPE_3AA_FRONT));
    ...
    sensorIds[CAPTURE_NODE] = (0 << REPROCESSING_SHIFT)
                   ...
                   | (sensorId << 0);
    sensorIds[SUB_NODE] = -1;
    ret = m_pipes[INDEX(PIPE_3AC_FRONT)]->create(sensorIds);
    if (ret < 0) {
        ALOGE("ERR(%s[%d]): 3AC create fail, ret(%d)", __FUNCTION__, __LINE__, ret);
        /* TODO: exception handling */
        return INVALID_OPERATION;
    }
    ALOGD("DEBUG(%s):Pipe(%d) created", __FUNCTION__, INDEX(PIPE_3AC_FRONT));
...
status_t ExynosCameraFrameFactoryFront::m_fillNodeGroupInfo(ExynosCameraFrame *frame)
{
    ...
    node_group_info_3aa.capture[PERFRAME_FRONT_3AP_POS].request = m_request3AP;
...
{
    int ret = 0;
    ExynosCameraFrameEntity *newEntity[INDEX(MAX_PIPE_NUM_FRONT)];
    ExynosCameraFrame *frame = new ExynosCameraFrame(m_parameters, m_frameCount);
    ...
    newEntity[INDEX(PIPE_3AA_FRONT)] = new ExynosCameraFrameEntity(PIPE_3AA_FRONT, ENTITY_TYPE_INPUT_OUTPUT, ENTITY_BUFFER_FIXED);
    frame->addChildEntity(newEntity[INDEX(PIPE_FLITE_FRONT)], newEntity[INDEX(PIPE_3AA_FRONT)]);
    ...
    frame->addChildEntity(newEntity[INDEX(PIPE_3AA_FRONT)], newEntity[INDEX(PIPE_ISP_FRONT)]);
    requestEntityCount++;
    ...
    newEntity[INDEX(PIPE_SCP_FRONT)] = new ExynosCameraFrameEntity(PIPE_SCP_FRONT, ENTITY_TYPE_OUTPUT_ONLY, ENTITY_BUFFER_DELIVERY);
...
status_t ExynosCameraFrameFactoryFront::initPipes(void)
{
    ...
    pipeInfo[0].bufInfo.count = FRONT_NUM_BAYER_BUFFERS;
    /* per frame info */
    pipeInfo[0].perFrameNodeGroupInfo.perframeSupportNodeNum = 2; /* 3A1, 3A1P */
    ...
    pipeInfo[0].perFrameNodeGroupInfo.perFrameLeaderInfo.perFrameVideoID = (FRONT_CAMERA_3AA_NUM - FIMC_IS_VIDEO_SS0_NUM);
    pipeInfo[0].perFrameNodeGroupInfo.perFrameCaptureInfo[0].perFrameNodeType = PERFRAME_NODE_TYPE_CAPTURE;
    pipeInfo[0].perFrameNodeGroupInfo.perFrameCaptureInfo[0].perFrameVideoID = (FRONT_CAMERA_3AP_NUM - FIMC_IS_VIDEO_SS0_NUM);
    ...
    pipeInfo[1].bufInfo.count = FRONT_NUM_BAYER_BUFFERS;
    /* per frame info */
    ...
    ret = m_pipes[INDEX(PIPE_3AA_FRONT)]->setupPipe(pipeInfo);
    if (ret < 0) {
    ...
    if (m_parameters->checkBayerDumpEnable()) {
        /* packed bayer bytesPerPlane */
        pipeInfo[0].bytesPerPlane[0] = ROUND_UP(pipeInfo[0].rectInfo.fullW * 2, 16);
    }
    else
#endif
    {
...
status_t ExynosCameraFrameFactoryFront::startPipes(void)
{
    ...
    /* stream on for 3AA */
    ret = m_pipes[INDEX(PIPE_3AA_FRONT)]->start();
    if (ret < 0) {
        ALOGE("ERR(%s[%d]):3AA start fail, ret(%d)", __FUNCTION__, __LINE__, ret);
        /* TODO: exception handling */
        return INVALID_OPERATION;
    }
...
status_t ExynosCameraFrameFactoryFront::startInitialThreads(void)
{
    ...
    ret = startThread(PIPE_3AA_FRONT);
    if (ret < 0)
        return ret;
...
status_t ExynosCameraFrameFactoryFront::setStopFlag(void)
{
    ...
    ret = m_pipes[INDEX(PIPE_3AA_FRONT)]->setStopFlag();
...
status_t ExynosCameraFrameFactoryFront::stopPipes(void)
{
    ...
    /* stream off for 3AA */
    ret = m_pipes[INDEX(PIPE_3AA_FRONT)]->stopThread();
    if (ret < 0) {
        ALOGE("ERR(%s[%d]):3AA stopThread fail, ret(%d)", __FUNCTION__, __LINE__, ret);
        /* TODO: exception handling */
        return INVALID_OPERATION;
    }
    ...
    ret = m_pipes[INDEX(PIPE_FLITE_FRONT)]->stopThread();
    if (ret < 0) {
    ...
    /* stream off for 3AA */
    ret = m_pipes[INDEX(PIPE_3AA_FRONT)]->stop();
    if (ret < 0) {
        ALOGE("ERR(%s[%d]):3AA stop fail, ret(%d)", __FUNCTION__, __LINE__, ret);
        /* TODO: exception handling */
        return INVALID_OPERATION;
    }
