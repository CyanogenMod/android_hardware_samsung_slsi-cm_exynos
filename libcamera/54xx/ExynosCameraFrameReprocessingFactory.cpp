...
{
    int ret = 0;
    ExynosCameraFrameEntity *newEntity[INDEX(MAX_PIPE_NUM_REPROCESSING)];
    ...
    if (m_parameters->getUsePureBayerReprocessing() == true) {
        ...
        newEntity[INDEX(PIPE_3AA_REPROCESSING)] = new ExynosCameraFrameEntity(PIPE_3AA_REPROCESSING, ENTITY_TYPE_INPUT_OUTPUT, ENTITY_BUFFER_FIXED);
    ...
    if (newEntity[INDEX(PIPE_3AA_REPROCESSING)] == NULL)
        frame->addSiblingEntity(NULL, newEntity[INDEX(PIPE_ISP_REPROCESSING)]);
    else
        frame->addChildEntity(newEntity[INDEX(PIPE_3AA_REPROCESSING)], newEntity[INDEX(PIPE_ISP_REPROCESSING)]);
    requestEntityCount++;

    /* set SCC pipe to linkageList */
    newEntity[INDEX(PIPE_SCC_REPROCESSING)] = new ExynosCameraFrameEntity(PIPE_SCC_REPROCESSING, ENTITY_TYPE_OUTPUT_ONLY, ENTITY_BUFFER_FIXED);
    frame->addSiblingEntity(NULL, newEntity[INDEX(PIPE_SCC_REPROCESSING)]);
    requestEntityCount++;
...
status_t ExynosCameraFrameReprocessingFactory::initPipes(void)
{
    ...
    if (m_parameters->getUsePureBayerReprocessing() == true) {
        ...
        if (m_parameters->checkBayerDumpEnable()) {
            ...
            pipeInfo[0].bytesPerPlane[0] = ROUND_UP(pipeInfo[0].rectInfo.fullW, 16) * 2;
        }
        else
#endif
        {
