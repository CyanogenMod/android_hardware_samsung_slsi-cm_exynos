...
status_t ExynosCameraParameters::checkFastFpsMode(const CameraParameters& params)
{
    ...
    fastFpsMode = -1;
    ALOGD("DEBUG(%s):fast-fps-mode not supported: %d", "setParameters", fastFpsMode);
#endif
    ...
    if ((recordingHint == true) && !(isShotModeAnimated)) {
        ...
        ALOGE("INFO(%s):fastFpsMode(%d) prevFpsMode(%d)", "checkFastFpsMode", fastFpsMode, prevFpsMode);
        setReallocBuffer(true);
        m_setRestartPreview(true);
                                                     ...
                                                     int *newCalPreviewW, int *newCalPreviewH)
{
    ...
    if (getHighSpeedRecording() == true) {
        ...
        if (m_getHighSpeedRecordingSize(sizeList) == NO_ERROR) {
            *newPreviewW = sizeList[TARGET_W];
            *newPreviewH = sizeList[TARGET_H];
        } else {
    ...
    if (m_isHighResolutionCallbackSize(*newPreviewW, *newPreviewH) == true) {
...
status_t ExynosCameraParameters::m_getHighSpeedRecordingSize(int *sizeList)
{
    uint32_t min = 0;
    uint32_t max = 0;
    int videoW = 0;
    ...
    if( fpsmode <= 0 ){
        ALOGE("DEBUG(%s[%d]):getFastFpsMode fpsmode(%d) fail", __FUNCTION__, __LINE__, fpsmode);
        return BAD_VALUE;
    } else {
        fpsmode--;
    }
    ...
    for(int i = 0 ; i < SIZE_LUT_INDEX_END ; i++) {
        sizeList[i] = m_staticInfo->videoSizeLutHighSpeed[fpsmode][i];
    }
...
{
    ALOGV("DEBUG(%s):getPreviewBufferCount %d", "setParameters", m_previewBufferCount);
...
void ExynosCameraParameters::setPreviewBufferCount(int previewBufferCount)
{
    ...
    ALOGV("DEBUG(%s):setPreviewBufferCount %d", "setParameters", m_previewBufferCount);
...
/* HACK: enable when FW ready */
#if 0
        } else if (m_isUHDRecordingMode() == true) {
...
bool ExynosCameraParameters::getUseDynamicScc(void)
{
    ...
    bool reprocessing = isReprocessing(getCameraId());
...
int32_t ExynosCameraParameters::getReprocessingBayerMode(void)
{
    ...
    if (isReprocessing(getCameraId()) == false)
        return mode;
...
bool ExynosCameraParameters::checkBayerDumpEnable(void)
{
    ...
    if (strcmp(enableRawDump, "1") == 0) {
        /*ALOGD("checkBayerDumpEnable : 1");*/
        return true;
    } else {
        /*ALOGD("checkBayerDumpEnable : 0");*/
        return false;
    }
}
#endif  /* DEBUG_RAWDUMP */
