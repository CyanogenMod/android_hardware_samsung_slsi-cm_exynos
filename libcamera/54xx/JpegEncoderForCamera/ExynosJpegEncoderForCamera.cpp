...
int ExynosJpegEncoderForCamera::encode(int *size, exif_attribute_t *exifInfo, debug_attribute_t *debugInfo)
{
    int ret = ERROR_NONE;
    ...
    if (exifInfo != NULL) {
        ...
        if (debugInfo != NULL && debugInfo->debugData != NULL) {
            ...
            ALOGD("AppMarker buf size = %d", bufSize);
            ALOGD("AppMarker debug size = %d", debugInfo->debugSize);
        }
        ...
        if (exifLen <= EXIF_LIMIT_SIZE ||
            ...
            ALOGI("INFO(%s[%d]): memmove size:%d", __FUNCTION__, __LINE__, iJpegSize - 2);
